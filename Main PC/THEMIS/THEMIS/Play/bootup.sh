...
# initialize
read -p $'\e[31mATTENTION: Press START to initialize THEMIS! Limbs will move!\e[0m'
screen -S themis -p auxiliary -X stuff 'python3 -m Play.Others.initialize^M'
sleep 2s
echo '====== THEMIS Initialized ======'
echo ''

# run_sense
read -p $'\e[31mATTENTION: Place THEMIS on the ground! Press ENTER to start THEMIS sense!\e[0m'
...
sleep 1s
read -p $'\e[31mATTENTION: Press ENTER to start low-level control! THEMIS may twitch a bit!\e[0m'
sleep 1s
...
# high_level
read -p $'\e[31mATTENTION: Press ENTER to start high-level control! THEMIS will rise a bit!\e[0m'
sleep 1s
