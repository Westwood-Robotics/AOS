...
from Play.Locomotion.command_macros import *
...
class UserCommand(object):
    ...
    def update(self):
        ...
        if time.perf_counter() - self.last_display_time > self.display_dt:
            self.last_display_time = time.perf_counter()
            self.display()
        ...
        if self.controller == 'keyboard':
            ...
            if Themis.gamepad['ST']:
                ...
                if self.mode == BALANCE:
                    ...
                    if (Themis.gamepad['LSZ'] or Themis.gamepad['RSP']) and PARAMETER_MAX[BASE_ORIENTATION_Z] - self.parameter[BASE_ORIENTATION_Z]['value_raw'] > self.tol:
                        self.parameter[BASE_ORIENTATION_Z]['value_raw'] += PARAMETER_INCREMENT[BASE_ORIENTATION_Z]
                    elif (Themis.gamepad['RSZ'] or Themis.gamepad['RSM']) and self.parameter[BASE_ORIENTATION_Z]['value_raw'] - PARAMETER_MIN[BASE_ORIENTATION_Z] > self.tol:
                        self.parameter[BASE_ORIENTATION_Z]['value_raw'] -= PARAMETER_INCREMENT[BASE_ORIENTATION_Z]
                else:
                    ...
                    # if Themis.gamepad['RSP']:
                    #     self.parameter[BASE_YAW_RATE]['value_raw'] = PARAMETER_MAX[BASE_YAW_RATE]
                    # elif Themis.gamepad['RSM']:
                    #     self.parameter[BASE_YAW_RATE]['value_raw'] = PARAMETER_MIN[BASE_YAW_RATE]
                    # else:
                    #     self.parameter[BASE_YAW_RATE]['value_raw'] = 0
    ...
    def set_command(self):
        ...
        for idx in PARAMETER_ID_LIST:
            ...
            # self.parameter[idx]['value_filter'] = MF.linear_filter(self.parameter[idx]['value_filter'], self.parameter[idx]['value'], PARAMETER_INCREMENT[idx] / loop_freq * 10)
        input_data = {'locomotion_mode':         np.array([self.mode]),
                      'horizontal_velocity':     np.zeros(2) if self.mode == BALANCE else np.array([self.parameter[COM_VELOCITY_X]['value_filter'], self.parameter[COM_VELOCITY_Y]['value_filter']]) + np.array(COM_VELOCITY_COMPENSATION[self.mode]),
                      'yaw_rate':                np.zeros(1) if self.mode == BALANCE else np.array([self.parameter[BASE_YAW_RATE]['value_filter']]) * DEG2RAD,
    ...
    def display(self):
        ...
        arm_r_data = MM.RIGHT_ARM_JOINT_STATE.get()
        arm_l_data = MM.LEFT_ARM_JOINT_STATE.get()
        head_data  = MM.HEAD_JOINT_STATE.get()

        vol_leg_r_msg, vol_leg_l_msg, vol_arm_r_msg, vol_arm_l_msg, vol_head_msg = " ", " ", " ", " ", " "
        tem_leg_r_msg, tem_leg_l_msg, tem_arm_r_msg, tem_arm_l_msg, tem_head_msg = " ", " ", " ", " ", " "
        for idx in range(6):
            vol_leg_r_i = leg_r_data['bear_voltages'][idx]
            vol_leg_r_msg += (colored(str(vol_leg_r_i)[0:4], 'red') if vol_leg_r_i < 24 else str(vol_leg_r_i)[0:4]) + " "

            vol_leg_l_i = leg_l_data['bear_voltages'][idx]
            vol_leg_l_msg += (colored(str(vol_leg_l_i)[0:4], 'red') if vol_leg_l_i < 24 else str(vol_leg_l_i)[0:4]) + " "

            tem_leg_r_i = leg_r_data['bear_temperatures'][idx]
            tem_leg_r_msg += (colored(str(tem_leg_r_i)[0:4], 'red') if vol_leg_r_i > 75 else str(tem_leg_r_i)[0:4]) + " "

            tem_leg_l_i = leg_l_data['bear_temperatures'][idx]
            tem_leg_l_msg += (colored(str(tem_leg_l_i)[0:4], 'red') if tem_leg_l_i > 75 else str(tem_leg_l_i)[0:4]) + " "

        for idx in range(6):
            vol_arm_r_i = arm_r_data['bear_voltages'][idx]
            vol_arm_r_msg += (colored(str(vol_arm_r_i)[0:4], 'red') if vol_arm_r_i < 24 else str(vol_arm_r_i)[0:4]) + " "

            vol_arm_l_i = arm_l_data['bear_voltages'][idx]
            vol_arm_l_msg += (colored(str(vol_arm_l_i)[0:4], 'red') if vol_arm_l_i < 24 else str(vol_arm_l_i)[0:4]) + " "

            tem_arm_r_i = arm_r_data['bear_temperatures'][idx]
            tem_arm_r_msg += (colored(str(tem_arm_r_i)[0:4], 'red') if tem_arm_r_i > 75 else str(tem_arm_r_i)[0:4]) + " "

            tem_arm_l_i = arm_l_data['bear_temperatures'][idx]
            tem_arm_l_msg += (colored(str(tem_arm_l_i)[0:4], 'red') if tem_arm_l_i > 75 else str(tem_arm_l_i)[0:4]) + " "

        for idx in range(2):
            vol_head_i = head_data['bear_voltages'][idx]
            vol_head_msg += (colored(str(vol_head_i)[0:4], 'red') if vol_head_i < 24 else str(vol_head_i)[0:4]) + " "

            tem_head_i = head_data['bear_temperatures'][idx]
            tem_head_msg += (colored(str(tem_head_i)[0:4], 'red') if tem_head_i > 75 else str(tem_head_i)[0:4]) + " "

        os.system('clear')
        print("====== The User Input Thread is running at", loop_freq, "Hz... ======")
        print()
        print("BEAR Voltage [V]:     ", vol_leg_r_msg, vol_leg_l_msg)
        print("                      ", vol_arm_r_msg, vol_arm_l_msg)
        print("                      ", vol_head_msg)
        print("BEAR Temperature [°C]:", tem_leg_r_msg, tem_leg_l_msg)
        print("                      ", tem_arm_r_msg, tem_arm_l_msg)
        print("                      ", tem_head_msg)
        print("_________")
        print()
    ...
    try:
        while True:
            ...
            if last_check_thread_elapse > check_thread_duration:
                ...
                if Themis.thread_issue(thread_name='top_level'):
                    Themis.stop_threading()
