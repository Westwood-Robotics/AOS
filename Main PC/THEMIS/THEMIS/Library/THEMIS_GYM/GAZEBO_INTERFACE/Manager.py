...
class GazeboInterfaceManager(object):
    ...
    def initialize_shared_memory(self):
        self.WORLD_PARAMETER = shared_memory_manager.SharedMemoryManager(robot_name='GAZ', seg_name='WORLD_PARAMS', init=False)
        self.WORLD_PARAMETER.add_block(name='data', data=np.array(WorldParameters))

        self.MODEL_PARAMETER = shared_memory_manager.SharedMemoryManager(robot_name=self.robot_name, seg_name='MODEL_PARAMS', init=False)
        self.MODEL_PARAMETER.add_block(name='data', data=np.array(ModelParameters))

        self.JOINT_STATE = shared_memory_manager.SharedMemoryManager(robot_name=self.robot_name, seg_name='STATES', init=False)
        ...
        self.JOINT_TORQUE_COMMAND = shared_memory_manager.SharedMemoryManager(robot_name=self.robot_name, seg_name='FORCE_COMMS', init=False)
        self.JOINT_TORQUE_COMMAND.add_block(name='data', data=np.zeros(self.num_joints))

        self.POSITION_PID_GAIN = shared_memory_manager.SharedMemoryManager(robot_name=self.robot_name, seg_name='PID_GAINS', init=False)
        self.POSITION_PID_GAIN.add_block(name='data', data=np.zeros(3 * self.num_joints))

        self.JOINT_POSITION_COMMAND = shared_memory_manager.SharedMemoryManager(robot_name=self.robot_name, seg_name='POS_COMMS', init=False)
        self.JOINT_POSITION_COMMAND.add_block(name='data', data=np.zeros(self.num_joints))

        self.JOINT_LIMIT = shared_memory_manager.SharedMemoryManager(robot_name=self.robot_name, seg_name='JOINT_LIMITS', init=False)
        self.JOINT_LIMIT.add_block(name='data', data=np.zeros(2 * self.num_joints))

        self.TORQUE_LIMIT = shared_memory_manager.SharedMemoryManager(robot_name=self.robot_name, seg_name='EFFORT_LIMITS', init=False)
        self.TORQUE_LIMIT.add_block(name='data', data=np.zeros(2 * self.num_joints))

        self.BODY_POSE = shared_memory_manager.SharedMemoryManager(robot_name=self.robot_name, seg_name='BODY_POSE', init=False)
        ...
        self.IMU_STATE = shared_memory_manager.SharedMemoryManager(robot_name=self.robot_name, seg_name='IMU_STATES', init=False)
        ...
        self.LIMB_CONTACT = shared_memory_manager.SharedMemoryManager(robot_name=self.robot_name, seg_name='LIMB_CONTACTS', init=False)
        self.LIMB_CONTACT.add_block(name='on', data=np.zeros(self.num_contact_sensors))

        self.BODY_FORCE = shared_memory_manager.SharedMemoryManager(robot_name=self.robot_name, seg_name='BODY_FORCE', init=False)
        self.BODY_FORCE.add_block(name='force', data=np.zeros(3))

        self.BODY_TORQUE = shared_memory_manager.SharedMemoryManager(robot_name=self.robot_name, seg_name='BODY_TORQUE', init=False)
